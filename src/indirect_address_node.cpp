 ...
 *
 * Open terminal #2
 * $ rosrun dynamixel_sdk_robot indirect_address_node
 *
 * Open terminal #3 (run one of below commands at a time)
 * $ rostopic pub -1 /sync_set_position dynamixel_sdk_robot/SyncSetPosition "{id1: 1, id2: 2, position1: 0, position2: 1000}"
 * $ rostopic pub -1 /sync_set_position dynamixel_sdk_robot/SyncSetPosition "{id1: 1, id2: 2, position1: 1000, position2: 0}"
...
#define ADDR_INDIRECT_ADDR1   168
#define ADDR_INDIRECT_ADDR5   176
#define ADDR_INDIRECT_DATA1   224
#define ADDR_INDIRECT_DATA5   228
  ...
  dynamixel_sdk_robot::SyncGetPosition::Response & res)
{
  ...
  if (dxl_comm_result == COMM_SUCCESS) {
    position1 = groupSyncRead.getData((uint8_t)req.id1, ADDR_INDIRECT_DATA1, 4);
    position2 = groupSyncRead.getData((uint8_t)req.id2, ADDR_INDIRECT_DATA1, 4);
...
int main(int argc, char ** argv)
{
  ...
  for (uint8_t i = 0; i < 4; i++) {
    dxl_comm_result = packetHandler->write2ByteTxRx(
      portHandler, DXL1_ID, ADDR_INDIRECT_ADDR1 + i * 2, ADDR_PRESENT_POSITION + i, &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS) {
      ROS_ERROR("Failed to set indirect address for Dynamixel ID %d", DXL1_ID);
      return -1;
    }

    dxl_comm_result = packetHandler->write2ByteTxRx(
      portHandler, DXL1_ID, ADDR_INDIRECT_ADDR5 + i * 2, ADDR_GOAL_POSITION + i, &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS) {
      ROS_ERROR("Failed to set indirect address for Dynamixel ID %d", DXL1_ID);
      return -1;
    }
    ...
    dxl_comm_result = packetHandler->write2ByteTxRx(
      portHandler, DXL2_ID, ADDR_INDIRECT_ADDR1 + i * 2, ADDR_PRESENT_POSITION + i, &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS) {
      ROS_ERROR("Failed to set indirect address for Dynamixel ID %d", DXL2_ID);
      return -1;
    }

    dxl_comm_result = packetHandler->write2ByteTxRx(
      portHandler, DXL2_ID, ADDR_INDIRECT_ADDR5 + i * 2, ADDR_GOAL_POSITION + i, &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS) {
      ROS_ERROR("Failed to set indirect address for Dynamixel ID %d", DXL2_ID);
      return -1;
    }
  ...
  ros::init(argc, argv, "indirect_address_node");
  ros::NodeHandle nh;
